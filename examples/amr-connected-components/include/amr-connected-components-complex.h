...
void send_edges_to_neighbors_cc(FabComponentBlock<R, D>* b, const diy::Master::ProxyWithLink& cp)
{
    ...
    for(const diy::BlockID& receiver : link_unique(l, b->gid))
    {
        ...
        //fmt::print("send_edges_to_neighbors_cc: r.gid = {}, r.proc = {}\n", receiver.gid, receiver.proc);
        if (debug)
            fmt::print("In send_edges_to_neighbors for block = {}, sending to receiver= {}, cardinality = {}\n", b->gid,
                    receiver_gid, b->gid_to_outgoing_edges_[receiver_gid].size());
    }
}
...
//    bool debug = (b->gid == 3) || (b->gid == 11) || (b->gid == 0) || (b->gid == 1);
    ...
    if (debug)
        fmt::print(
                "In expand_link for block = {}, round = {}, b->done_ = {}, n_added = {}, new link size = {}, new link size_unique = {}\n",
...
void amr_cc_send(FabComponentBlock<Real, D>* b, const diy::Master::ProxyWithLink& cp)
{
    ...
    for(const diy::BlockID& receiver : receivers)
    {
        ...
        {
            if (debug)
                fmt::print("in amr_cc_send, c = {}, receiver gid = {} START\n", c.original_deepest(), receiver_gid);
...
//            }
            ...
            if (debug)
                fmt::print("in amr_cc_send, c = {}, receiver gid = {}, #current_neighbors = {}, #edges = {}\n",
            ...
            if (debug)
                fmt::print("in amr_cc_send, c = {}, receiver gid = {}, sent original deepest, n_trees = {}\n",
...
//        assert(n_sent == n_components);
    ...
    if (debug)
        fmt::print("Exit send_components for block = {}, b->done = {}, b->round = {}\n", b->gid, b->done_, b->round_);
}
...
//    DurationType expand_link_time;
    ...
    for(const diy::BlockID& sender : senders)
    {
        ...
        if (debug)
            fmt::print("amr_cc_receive, round = {}, block {}, receiving from {}\n", b->round_, b->gid, sender.gid);
...
//          if (debug) fmt::print("HERE c.original_deepest() == {}\n", c.original_deepest());
...
//    int old_size = l->size();
    ...
    if (debug)
        ...
        {
            fmt::print(
                    "amr_cc_receive END: Component: {}, #current_gids = {}, #processed_gids = {}, must_send_neighbors = {}\n",
    ...
    if (debug)
        fmt::print("Exit amr_cc_receive, gid = {}, round = {}, mt.size = {}, done = {}\n", b->gid, b->round_,
                b->get_merge_tree().size(), b->done_);
