...
size_t get_vertex_id(const diy::Point<int, D>& v, int v_refinement, int link_idx, diy::AMRLink* l, bool c_order)
{
    ...
    vv -= from;
    if (debug)
    { fmt::print("in local coords vv = {}\n", vv); }
    if (debug)
    { fmt::print("index = {}\n", grid.index(vv)); }

    if (not(0 <= grid.index(vv) and grid.index(vv) < grid.size()))
    {
        fmt::print("Error here, v = {}, v_refinement = {}, link_idx = {}, from = {}, to = {}, vv = {}, result = {}\n",
                v, v_refinement, link_idx, from, to, vv, grid.index(vv));
 ...
 * @param neighbor_idx index of neighbor in link
 * @param l AMRLink
 ...
 */
template<unsigned D>
bool neighbor_contains(size_t neighbor_idx, diy::AMRLink* l, diy::Point<int, D>& v_glob, int v_refinement)
{
    using Position = diy::Point<int, D>;

    Position from = point_from_dynamic_point<D>(l->core(neighbor_idx).min);
    Position to = point_from_dynamic_point<D>(l->core(neighbor_idx).max);
    // TODO: vector refinement
    int refinement = l->refinement(neighbor_idx)[0];
    ...
    } else if (v_refinement > refinement)
    {
        ...
        if (debug)
        { fmt::print("In NeighborBox.contains, scale = {}\n", scale); }
        ...
        for(size_t i = 0; i < D; ++i)
            ...
            {
                if (debug)
                { fmt::print("In NeighborBox.contains, coord = {}, does NOT contain, return false\n", i); }
                return false;
            }
    } else
    {
    ...
    if (debug)
    { fmt::print("In neighbor_contains, DOES contain, return true\n"); }
    return true;
}
